...
class ZaloAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self._last_sender: Dict[str, str] = {}  # chat_id -> sender_id for auto-@ reply in groups
    ...
    async def _handle_sse_event(self, event_type: str, payload: str) -> None:
        ...
        if event_type in ("reaction", "undo", "friend_event"):
            logger.info("Zalo: %s event %s", event_type, data)
            return

        # ── Group join event: chào mừng thành viên mới ───────────────────────
        if event_type == "group_event":
            logger.info("Zalo: raw group_event data: %s", data)
            if data.get("type") == "join":
                group_data = data.get("data", {}) or {}
                thread_id = str(data.get("threadId") or group_data.get("groupId") or "")
                group_name = group_data.get("groupName") or "nhóm này"
                new_members = group_data.get("updateMembers") or []
                if thread_id and new_members:
                    # Lấy tất cả uid hợp lệ
                    uids = [str(m.get("id") or "") for m in new_members if m.get("id")]
                    uids = [u for u in uids if u]
                    if uids:
                        # ponytail: chào tất cả member mới trong 1 tin, mỗi @member pos cách nhau 8 (7 + 1 space), ASCII nên byte==char
                        if len(uids) == 1:
                            text = f"@member ơi! 🥳💖\nChào mừng bạn đã đến với {group_name}! ✨🎉\nRất vui được chào đón bạn 🤗🌸 Chúc bạn có thật nhiều niềm vui, chia sẻ những điều thú vị và cùng mọi người tạo nên những kỷ niệm đáng nhớ nhé! 😘💫🎈🫶"
                            mentions = [{"pos": 0, "uid": uids[0], "len": 7}]
                        else:
                            prefix = " ".join(["@member"] * len(uids))
                            text = f"{prefix} ơi! 🥳💖\nChào mừng các bạn đã đến với {group_name}! ✨🎉\nRất vui được chào đón các bạn 🤗🌸 Chúc mọi người có thật nhiều niềm vui và kỷ niệm đáng nhớ nhé! 😘💫🎈🫶"
                            mentions = []
                            pos = 0
                            for uid in uids:
                                mentions.append({"pos": pos, "uid": uid, "len": 7})
                                pos += 8  # "@member" 7 + space 1
                        await self._post(
                            "/send",
                            {
                                "threadId": thread_id,
                        ...
                        logger.info("Zalo: chào mừng thành viên mới %s vào nhóm %s (%s)", uids, thread_id, group_name)
                return
    ...
    async def _on_inbound_message(self, m: Dict[str, Any]) -> None:
        ...
        # Remember last sender per thread for auto-@ on reply (dynamic tag, 100 users OK).
        if sender_id and thread_type == "group":
            self._last_sender[thread_id] = sender_id
        ...
        # ── Zalo-specific system prompt: chặn bot viết code, chỉ trả kết quả ────────
        ZALO_DENY_CODE_PROMPT = (
            "[QUAN TRỌNG] Khi trả lời từ Zalo, BẮT BUỘC tuân thủ:\n"
            "1. CHỈ trả kết quả trực tiếp (list, link, số liệu...). Tuyệt đối KHÔNG viết code Python/Node/bash/shell.\n"
            "2. Dùng Unicode tiếng Việt chuẩn UTF-8 (có dấu).\n"
            "3. Ngắn gọn, 1-3 câu, tối đa 10 dòng.\n"
            "4. Nếu người dùng hỏi cách làm code → mới được viết. Ngược lại → chỉ kết quả.\n"
            "5. LỊCH SỰ BẮT BUỘC: luôn dạ/vâng/ạ, xưng mình/bạn hoặc em/anh/chị lễ phép. TUYỆT ĐỐI KHÔNG nói bậy, chửi thề, teencode tục.\n"
            "6. Giọng tự nhiên như người thật, emoji 1-2 cái thôi (😊 ✨).\n"
        )
        if chat_type in ("group", "dm"):
            existing = ""
            try:
                existing = getattr(self._message_handler, "_system_prompt", "") or ""
            except Exception:
                existing = ""
            if ZALO_DENY_CODE_PROMPT not in existing:
                try:
                    self._message_handler._system_prompt = existing + "\n" + ZALO_DENY_CODE_PROMPT
                except Exception:
                    pass
    ...
    async def send(
        ...
        last = None
        # Auto-@ sender in groups: prepend @member + mentions so bot tags đúng người hỏi (dynamic per chat_id).
        auto_mention_uid: Optional[str] = None
        if thread_type == "group":
            auto_mention_uid = self._last_sender.get(str(chat_id))
        for idx, chunk in enumerate(chunks):
            if not chunk.strip():
                continue
            body: Dict[str, Any] = {"threadId": chat_id, "threadType": thread_type, "text": chunk}
            if auto_mention_uid and idx == 0 and chunk:
                # Prepend @member so Zalo renders blue tag; pos 0 len 7 matches "@member"
                body["text"] = f"@member {chunk}"
                body["mentions"] = [{"pos": 0, "uid": auto_mention_uid, "len": 7}]
