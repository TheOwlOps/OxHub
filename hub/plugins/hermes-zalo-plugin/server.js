...
// ── Mention tracker (for zalouser allowlist capture) ────────────────────────
const MENTION_STORAGE = path.join(path.dirname(credentialsPath()), "mentioned_uids.json");
function loadMentions() {
	try {
		return JSON.parse(fs.readFileSync(MENTION_STORAGE, "utf-8"));
	} catch {
		return { uids: [], last_updated: "" };
	}
}
function saveMentions(data) {
	data.last_updated = new Date().toISOString();
	try {
		fs.writeFileSync(MENTION_STORAGE, JSON.stringify(data, null, 2), "utf-8");
	} catch {}
}

client.on("message", (msg) => {
	pushEvent("message", msg);
	// Capture @mentions in group messages
	try {
		const isGroup = msg.isGroup === true;
		const data = msg.data && typeof msg.data === "object" ? msg.data : {};
		const mentions = Array.isArray(data.mentions) ? data.mentions : [];
		if (isGroup && mentions.length > 0) {
			const storage = loadMentions();
			for (const mn of mentions) {
				const uid = String(mn.uid || mn.userId || mn.id).trim();
				if (uid && !storage.uids.includes(uid)) storage.uids.push(uid);
			}
			if (storage.uids.length > 0) {
				saveMentions(storage);
				console.log(
					"[mention-tracker] Captured UIDs:",
					mentions.map((m) => m.uid || m.userId),
				);
			}
		}
	} catch (e) {
		console.error("[mention-tracker] capture failed:", e.message);
	}
});
...
// Prevent bad JSON from crashing the bridge (e.g. bad escaped char in body)
...
// Hard allowlist: bot ONLY replies in these thread IDs
const ALLOWED_THREADS = new Set(["8813682027038154228"]);

function checkAuth(req, res) {
	if (!TOKEN) return true;
	const provided =
		req.get("x-bridge-token") || (req.get("authorization") || "").replace(/^Bearer\s+/i, "") || req.query.token;
...
app.post("/send", async (req, res) => {
	...
	// Hard allowlist: bot ONLY replies in allowed threads
	if (!ALLOWED_THREADS.has(threadId)) {
		console.log("[zalo] ignoring message from non-allowlisted thread:", threadId);
...
//   paths: array of absolute file paths (Windows C:\\... OR Linux /c/...)
const USER_HOME = process.env.USERPROFILE || process.env.HOME || "C:\\Users\\OS";
function resolveFSPath(p) {
	// MSYS-style /c/... → C:\... (native Node on Windows can't see /c/)
	if (/^\/[a-zA-Z]\//.test(p)) {
		const win = p.replace(/^\/([a-zA-Z])\//, "$1:\\/").replace(/\//g, "\\");
		if (fs.existsSync(win)) return win;
	}
	// C:\... Windows absolute (already native)
	if (/^[a-zA-Z]:\\/.test(p) && fs.existsSync(p)) return p;
	// Relative or ~ → resolve against user home
	if (p.startsWith("~")) {
		const win = p.replace(/^~/, USER_HOME).replace(/\//g, "\\");
		if (fs.existsSync(win)) return win;
	}
...
app.post("/send-attachment", async (req, res) => {
	if (!checkAuth(req, res)) return;
	if (!requireLogin(res)) return;
	const { threadId, threadType = "user", paths, caption } = req.body || {};
	const rawPaths = req.body.paths || (req.body.path ? [req.body.path] : null);
	if (!threadId || !rawPaths || !rawPaths.length) {
		return res.status(400).json({ error: "threadId and paths required" });
	}
	// Resolve each path (Windows + MSYS-compatible)
	const resolvedPaths = rawPaths.map(resolveFSPath);
	for (const p of resolvedPaths) {
		if (!p) return res.status(400).json({ error: `file not found (tried Windows & /c/ forms)` });
	}
	try {
		const r = await client.sendAttachment(threadId, threadType, resolvedPaths, caption);
		res.json({ success: true, result: r });
	} catch (e) {
		res.status(500).json({ error: String(e && e.message ? e.message : e) });
	}
...
app.post("/group/add", async (req, res) => {
	...
	if (!groupId || !Array.isArray(members) || !members.length)
		return res.status(400).json({ error: "groupId and members[] required" });
	try {
...
// ── Member join detection (polling) ────────────────────────────────────────
const MEMBER_CACHE_FILE = path.join(path.dirname(credentialsPath()), "group_members_cache.json");
function loadMemberCache() {
	try {
		return JSON.parse(fs.readFileSync(MEMBER_CACHE_FILE, "utf-8"));
	} catch {
		return {};
	}
}
function saveMemberCache(data) {
	try {
		fs.writeFileSync(MEMBER_CACHE_FILE, JSON.stringify(data, null, 2), "utf-8");
	} catch {}
}

async function checkNewMembers() {
	const TARGET_GROUP = "8813682027038154228";
	const cache = loadMemberCache();
	const lastMembers = cache[TARGET_GROUP] || [];

	try {
		const info = await client.api.getGroupInfo(TARGET_GROUP);
		const groupInfo = info.gridInfoMap?.[TARGET_GROUP];
		const currentMembers = (groupInfo?.memberIds || []).map(String);

		if (lastMembers.length === 0) {
			// First run - just cache
			cache[TARGET_GROUP] = currentMembers;
		...
		if (newMembers.length > 0) {
			console.log("[welcome-poller] New members detected:", newMembers);
			// Update cache
			cache[TARGET_GROUP] = currentMembers;
			saveMemberCache(cache);

			// Send welcome message via bridge
			for (const uid of newMembers) {
				try {
					...
					console.log(`[welcome-poller] Sent welcome to ${uid}`);
				} catch (e) {
					console.error("[welcome-poller] Failed to send welcome:", e.message);
				}
			}
		} else {
			// Update cache if member count changed (someone left)
		...
		console.error("[welcome-poller] check failed:", e.message);
	}
}

// Run every 15 seconds
setInterval(checkNewMembers, 15000);
// Initial run
setTimeout(checkNewMembers, 5000);
